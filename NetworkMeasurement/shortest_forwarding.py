...
class Shortest_Forwarding(app_manager.RyuApp):
        ...
        if len(path) > 2:
            for i in xrange(1, len(path)-1):
                ...
                port_next = self.get_link_to_port(link_to_port,
                                                  path[i], path[i+1])
            	#self.test_delay(path[i],path[i+1])
	    	#thread.start_new_thread(self.test_delay, (2,3))
    ...
    def _packet_in_handler(self, ev):
        ...
        if eth_type == 0x07c3:
            ...
            self.total_time = receive_time-self.total_time
            print ("received!")
    ...
    def test_delay(self, src_dpid, dst_dpid):
        self.rtt_src['dpid'] = src_dpid
        self.rtt_src['time'] = time.time()
        self.send_meter_stats_request(self.datapaths[src_dpid])

        self.rtt_dst['dpid'] = dst_dpid
        self.rtt_dst['time'] = time.time()
        self.send_meter_stats_request(self.datapaths[dst_dpid])

        out_port = self.get_link_to_port(self.network_aware.link_to_port, src_dpid, dst_dpid)[0]
        p = self.build_packet(out_port)
        self.send_packet_out(self.datapaths[src_dpid], self.datapaths[src_dpid].ofproto.OFP_NO_BUFFER, self.datapaths[src_dpid].ofproto.OFPP_CONTROLLER, out_port, p.data)	
        self.total_time=time.time()
        time.sleep(1)

        print (self.total_time)
        print (self.total_time-(self.rtt_src['time']+self.rtt_dst['time'])/2)
        print ("success")
